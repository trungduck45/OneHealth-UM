...
@Component({
  selector: 'jhi-posterminal-modal',
  templateUrl: './posterminal-modal.component.html',
  styleUrls: ['./posterminal-modal.component.scss']
})
export class PosterminalModalComponent implements OnInit, OnChanges {
  @Input() inputId!: any;
  editing = false;
  ...
  REQUEST_URL = '/api/posterminal';
  POSPROFILE_REQUEST_URL = '/api/posprofile';
  listPosprofile: any[] = [];
  ...
  loadDataPosprofile(): void {
    this.dmService.query({}, this.POSPROFILE_REQUEST_URL).subscribe((response: HttpResponse<any>) => {
      if (response.body) {
        if (response.body.CODE === '00') {
          this.listPosprofile = response.body.RESULT.content;
        }
      }
    });
  }
