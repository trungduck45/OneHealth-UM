...
@Component({
  selector: 'jhi-cashier-modal',
  templateUrl: './cashier-modal.component.html',
  styleUrls: ['./cashier-modal.component.scss']
})
export class CashierModalComponent implements OnInit, OnChanges {
  @Input() inputId!: any;
  editing = false;
  ...
  REQUEST_URL = '/api/cashier';
  POSTERMINAL_REQUEST_URL = '/api/posterminal';
  listPosterminal: any[] = [];
  ...
  loadDataPosterminal(): void {
    this.dmService.query({}, this.POSTERMINAL_REQUEST_URL).subscribe((response: HttpResponse<any>) => {
      if (response.body) {
        if (response.body.CODE === '00') {
          this.listPosterminal = response.body.RESULT.content;
        }
      }
    });
  }
