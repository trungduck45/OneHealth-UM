...
@Component({
  selector: 'jhi-cloudsetting',
  templateUrl: './cloudsetting.component.html',
  styleUrls: ['./cloudsetting.component.scss']
})
export class CloudSettingComponent implements OnInit {
  editing = false;
  choosing = false;
  creating = false;
  ...
  private filter(): string {
    ...
    return comparesArray.length > 0 ? comparesArray.join(';') : 'code=="backup"';
  }
  ...
  openModal(entity: any): void {
    this.modalRef = this.modalService.open(CloudSettingModalComponent, {
